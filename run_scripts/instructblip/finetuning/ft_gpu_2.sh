...
python -c "import torch; print(torch.cuda.device_count())" >> test_gpu_n2.log 2>&1
# python -c "import torch; print(torch.version.cuda)" >> test_gpu_cuda.log 2>&1

python -m torch.distributed.run --nproc_per_node=2 train.py --cfg-path ./lavis/projects/instructblip/finetuning/alfworld_ft.yaml >> test_gpu_2.log 2>&1
