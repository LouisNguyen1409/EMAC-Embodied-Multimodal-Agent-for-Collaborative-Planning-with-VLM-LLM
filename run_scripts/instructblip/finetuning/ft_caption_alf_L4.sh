...
python -c "import torch; print(torch.cuda.device_count())" >> test_gpu_l4.log 2>&1
# python -c "import torch; print(torch.version.cuda)" >> test_gpu_cuda.log 2>&1

python -m torch.distributed.run --nproc_per_node=4 train.py --cfg-path ./lavis/projects/instructblip/finetuning/alfworld_ft.yaml >> test_l4.log 2>&1
