...
Model = Literal["gpt-4", "gpt-35-turbo", "gpt-35-turbo-16k", "text-davinci-003", "gpt-5.2-2025-12-11"]
...
def get_chat(prompt: str, model: Model, temperature: float = 0.0, max_tokens: int = 256, stop_strs: Optional[List[str]] = None) -> str:
    ...
    messages = [
        {
            "role": "system",
            "content": (
                "You are a text-completion engine for an interactive household task game. "
                "The user provides few-shot examples followed by the current game state. "
                "Output ONLY the next single action on one line, exactly matching the format "
                "in the examples (e.g. 'think: ...', 'go to shelf 1', 'take soapbar 1 from countertop 1', "
                "'put soapbar 1 in/on shelf 1', 'open cabinet 1', 'use desklamp 1', 'clean mug 1 with sinkbasin 1', "
                "'heat potato 1 with microwave 1', 'cool tomato 1 with fridge 1'). "
                "Do NOT include '>' prefix, explanations, commentary, or multiple actions. "
                "Just the single next action."
            )
        },
        {
            "role": "user",
    ...
    # Newer models (gpt-4o, gpt-5.2, etc.) require max_completion_tokens and don't support stop
    is_new_model = "gpt-5" in model or "gpt-4o" in model
    ...
    if not is_new_model and stop_strs:
        api_kwargs["stop"] = stop_strs
    response = openai.ChatCompletion.create(**api_kwargs)
    
    # print('Response: ', response)
    try:
        text = response["choices"][0]["message"]["content"]
        # For models that don't support stop sequences, manually truncate at first newline
        if is_new_model and stop_strs:
...
def llm_forward(prompt: str, model: Model, stop: List[str] = ["\n"]):
    ...
    while len(text.strip()) < 5 and cur_try < 6:
        if model == "text-davinci-003":
            ...
            text = get_chat(prompt=prompt, model=model, temperature=cur_try * 0.2, max_tokens=128, stop_strs=stop)
        cur_try += 1
